...
"""
from gtts import gTTS
from moviepy.editor import TextClip, concatenate_videoclips, CompositeAudioClip, AudioFileClip, CompositeVideoClip, ColorClip
import os
...
def create_text_clips(text, font, fontsize, color,bg_color, duration):
    ...
    for segment in text_segments:
        ...
        clips.append(clip)
    return clips, audio_clips
def generate_video(input_text, font, font_size, font_color='white', bg_color='black', duration=3, fps=3):
    # Set the font file
    font = "fonts/Nirmala.ttf"
    # Convert font size to float
    fontsize = float(font_size)
    
    # Create text and audio clips
    text_clips, audio_clips = create_text_clips(input_text, font, fontsize, font_color, bg_color, duration)
    video_duration = len(text_clips) * duration

    # Concatenate text clips into a single video clip
    final_clip = concatenate_videoclips(text_clips, method='compose')

    # Set the background color clip
    bg_clip = ColorClip(size=(final_clip.size[0], final_clip.size[1]), color=bg_color, duration=video_duration)

    # Composite the background color clip and the final video clip
    final_clip = CompositeVideoClip([bg_clip.set_duration(video_duration), final_clip.set_duration(video_duration)])
    ...
    for audio_clip in audio_clips:
        ...
        start_time += duration
    composite_audio_clip = CompositeAudioClip(adjusted_audio_clips)
    final_clip = final_clip.set_audio(composite_audio_clip)
    
    # Write the final video file
    final_clip.write_videofile("media/output_video_with_audio.mp4", fps=fps, codec='libx264')
